...
// ---------------------------------------------------------------------------
// Extended element data: electron config, electronegativity, uses
// ---------------------------------------------------------------------------
final Map<int, Map<String, String>> _elementExtras = {
  1:  {'config': '1s¹',                    'en': '2.20', 'uses': 'Fuel cells, rocket fuel, water production'},
  2:  {'config': '1s²',                    'en': '—',    'uses': 'Balloons, MRI cooling, welding'},
  3:  {'config': '[He] 2s¹',               'en': '0.98', 'uses': 'Batteries, ceramics, lubricants'},
  4:  {'config': '[He] 2s²',               'en': '1.57', 'uses': 'Aerospace alloys, X-ray windows'},
  5:  {'config': '[He] 2s² 2p¹',           'en': '2.04', 'uses': 'Glass, semiconductors, detergents'},
  6:  {'config': '[He] 2s² 2p²',           'en': '2.55', 'uses': 'Steel, diamonds, organic chemistry'},
  7:  {'config': '[He] 2s² 2p³',           'en': '3.04', 'uses': 'Fertilizers, explosives, food packaging'},
  8:  {'config': '[He] 2s² 2p⁴',           'en': '3.44', 'uses': 'Respiration, combustion, steel making'},
  9:  {'config': '[He] 2s² 2p⁵',           'en': '3.98', 'uses': 'Toothpaste, Teflon, refrigerants'},
  10: {'config': '[He] 2s² 2p⁶',           'en': '—',    'uses': 'Neon signs, lasers, cryogenics'},
  11: {'config': '[Ne] 3s¹',               'en': '0.93', 'uses': 'Table salt, street lights, soap'},
  12: {'config': '[Ne] 3s²',               'en': '1.31', 'uses': 'Alloys, fireworks, antacids'},
  13: {'config': '[Ne] 3s² 3p¹',           'en': '1.61', 'uses': 'Aircraft, packaging, electrical wiring'},
  14: {'config': '[Ne] 3s² 3p²',           'en': '1.90', 'uses': 'Semiconductors, solar cells, glass'},
  15: {'config': '[Ne] 3s² 3p³',           'en': '2.19', 'uses': 'Fertilizers, matches, DNA backbone'},
  16: {'config': '[Ne] 3s² 3p⁴',           'en': '2.58', 'uses': 'Sulfuric acid, rubber vulcanization'},
  17: {'config': '[Ne] 3s² 3p⁵',           'en': '3.16', 'uses': 'Water purification, PVC, bleach'},
  18: {'config': '[Ne] 3s² 3p⁶',           'en': '—',    'uses': 'Welding, light bulbs, lasers'},
  19: {'config': '[Ar] 4s¹',               'en': '0.82', 'uses': 'Fertilizers, gunpowder, glass'},
  20: {'config': '[Ar] 4s²',               'en': '1.00', 'uses': 'Bones, cement, chalk, antacids'},
  26: {'config': '[Ar] 3d⁶ 4s²',           'en': '1.83', 'uses': 'Steel, cast iron, magnets'},
  29: {'config': '[Ar] 3d¹⁰ 4s¹',          'en': '1.90', 'uses': 'Electrical wiring, coins, plumbing'},
  47: {'config': '[Kr] 4d¹⁰ 5s¹',          'en': '1.93', 'uses': 'Jewelry, photography, electronics'},
  79: {'config': '[Xe] 4f¹⁴ 5d¹⁰ 6s¹',    'en': '2.54', 'uses': 'Jewelry, electronics, dentistry'},
  82: {'config': '[Xe] 4f¹⁴ 5d¹⁰ 6s² 6p²','en': '2.33', 'uses': 'Batteries, radiation shielding'},
  92: {'config': '[Rn] 5f³ 6d¹ 7s²',       'en': '1.38', 'uses': 'Nuclear fuel, armor-piercing shells'},
};
...
class ElementDetailsSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final extras = _elementExtras[element.atomicNumber];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            element.symbol,
                            style: GoogleFonts.roboto(fontSize: 34, fontWeight: FontWeight.bold, color: color),
                          ),
                          Text(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Row(children: [
                            _miniStat('Period', element.period.toString(), color),
                            const SizedBox(width: 8),
                            _miniStat('Group', element.group == 0 ? 'f-block' : element.group.toString(), color),
                            const SizedBox(width: 8),
                            _miniStat('State', element.state, color),
                          ]),
                        ],
                      ),
                    ),
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Properties grid
                      _sectionTitle('Properties', isDark),
                      const SizedBox(height: 8),
                      _propGrid(isDark, [
                        ('Atomic Mass', '${element.atomicMass} u'),
                        ('Period', element.period.toString()),
                        ('Group', element.group == 0 ? 'f-block' : element.group.toString()),
                        ('State at STP', element.state),
                        if (extras != null) ...[ 
                          ('Electronegativity', extras['en'] ?? '—'),
                          ('Electron Config', extras['config'] ?? '—'),
                        ],
                      ]),

                      // Common uses
                      if (extras != null && (extras['uses'] ?? '').isNotEmpty) ...[
                        const SizedBox(height: 16),
                        _sectionTitle('Common Uses', isDark),
                        const SizedBox(height: 8),
                        Container(
  ...
  Widget _propGrid(bool isDark, List<(String, String)> props) {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              element.symbol,
              style: GoogleFonts.roboto(fontSize: 15, fontWeight: FontWeight.bold, color: color),
            ),
            Text(
              element.atomicMass < 100
                  ? element.atomicMass.toStringAsFixed(1)
                  : element.atomicMass.toStringAsFixed(0),
