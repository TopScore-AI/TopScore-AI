...
extension ChatControllerMessaging on ChatController {
  void handleIncomingMessage(dynamic dataRaw) {
    if (dataRaw is! Map<String, dynamic>) return;
    ...
    switch (type) {
      ...
      case 'title_updated':
        ...
        if (rawTitle != null && rawTitle.toString().isNotEmpty) {
          ...
          notify();
          // History update handled elsewhere
        }
        break;
      ...
      case 'chunk':
        ...
        if (targetId != null) {
          ...
          if (_chunkUpdateTimer?.isActive ?? false) return;
      ...
      case 'gamification':
        _totalXp = data['total_xp'] as int? ?? _totalXp;
        final prevStreak = _currentStreak;
        _currentStreak = data['current_streak'] as int? ?? _currentStreak;

        final alerts = data['animations_to_trigger'];
        if (alerts is List && alerts.isNotEmpty) {
          _gamificationAlerts =
              List<String>.from(alerts.map((e) => e.toString()));
          if (_gamificationAlerts.any(
              (a) => a.contains('Breakthrough') || a.contains('Unstoppable'))) {
            _showGamificationConfetti = true;
            _confettiController.play();
          }
          HapticFeedback.heavyImpact();
        } else if (data['xp_added'] != null && (data['xp_added'] as int) > 0) {
          HapticFeedback.lightImpact();
        }

        // Trigger sci-fi HUD overlay on streak milestones
        final streakMilestones = [7, 14, 30, 60, 100];
        if (streakMilestones.contains(_currentStreak) &&
            _currentStreak != prevStreak) {
          // LevelUpOverlay logic omitted as per existing code comment
        }
        notify();
        break;
      ...
      default:
        if (data.containsKey('sources')) handleSources(data);
        break;
    }
  }
    ...
    if (context != null) {
      ...
      if (uid != null) {
        context
            ...
            .record(uid, ActivityType.aiTutorMessage);
      ...
      if (idx != -1) {
        ...
        if (idx > 0) {
          ...
          if (userMsg.isUser) {
            ...
            if (userMsg.imageUrl != null) {
              _pendingFileUrl = userMsg.imageUrl;
              _pendingPreviewData = userMsg.imageUrl; 
              _pendingFileType = 'image'; // Fallback
              _isUploading = false;
            }
          }
        }
  ...
  Future<void> sendUserMessage(BuildContext context, {String? text, String? fileUrl, String? messageId}) async {
    final authProvider = context.read<AuthProvider>();
    if (!authProvider.canSendMessage) {
      if (authProvider.isGuestMode) {
        showDialog(
          context: context,
          builder: (context) => const GuestSavePromptDialog(),
        );
      } else {
        showDialog(
          context: context,
    ...
    String? fileUrlToSend = fileUrl ?? _pendingFileUrl;
    final String? fileTypeToSend = _pendingFileType;
    final Uint8List? fileBytes = _pendingFileBytes;
    final String? fileName = _pendingFileName;
    final String? previewData = _pendingPreviewData;
    ...
    final pendingMessage = ChatMessage(
      ...
      timestamp: DateTime.now(),
      imageUrl:
          fileUrlToSend ?? (fileTypeToSend == 'image' ? previewData : null),
    ...
    // ^ This placeholder is replaced or updated in handleIncomingMessage (response_start)
    ...
    try {
      if (fileBytes != null && fileUrlToSend == null) {
        if (fileTypeToSend != 'image') {
          final storageMime = fileTypeToSend == 'pdf'
              ? 'application/pdf'
              : 'application/octet-stream';
          fileUrlToSend = await uploadToFirebase(
              fileBytes, fileName ?? 'upload', storageMime);
          if (fileUrlToSend == null) throw Exception("Upload failed");

          final idx = _messages.indexWhere((m) => m.id == pendingId);
          if (idx != -1) {
            _messages[idx] = _messages[idx].copyWith(imageUrl: fileUrlToSend);
            notify();
      ...
      if (isBusy) {
        _messageQueue.add({
          ...
          'imageData': fileTypeToSend == 'image' ? previewData : null,
          'pendingId': pendingId,
        });
        return;
      }
  ...
  Future<void> regenerateResponse(BuildContext context, ChatMessage message,
    ...
    if (editedText != null) {
      ...
      notify();
      await sendUserMessage(context, text: editedText, messageId: message.id);
    } else {
      if (message.isUser) {
        ...
        notify();
        await sendUserMessage(context, text: message.text, fileUrl: message.imageUrl, messageId: message.id);
      } else {
        ...
        if (userIdx >= 0 && _messages[userIdx].isUser) {
          ...
          _messages.removeRange(idx, _messages.length);
          notify();
          await sendUserMessage(context, text: userMsg.text, fileUrl: userMsg.imageUrl, messageId: userMsg.id);
        }
      }
    }
  }
  ...
  void showRatingDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) => SessionRatingDialog(
        onSubmit: () => developer.log('Session Rated', name: 'ChatController'),
      ),
    );
  }
