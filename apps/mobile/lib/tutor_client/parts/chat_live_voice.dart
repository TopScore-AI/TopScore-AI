...
extension ChatControllerLiveVoice on ChatController {
      ...
      _liveGeminiEventSubscription = _geminiLiveService.events.listen((event) {
        switch (event.type) {
          case GeminiLiveEventType.error:
            ...
            _isLoading = false;
            notify();
            // Don't call stopLiveVoiceMode() immediately so the error overlay stays visible
            break;
          case GeminiLiveEventType.interrupted:
            developer.log('Live Voice: AI was interrupted (barge-in)');
            _audioOutput.flushBuffer(); // Instant audio stop
            break;
          case GeminiLiveEventType.turnComplete:
            developer.log('Live Voice: Turn complete');
            _resetSilenceTimer(); // Start silence detection after AI finishes
            break;
  ...
  Future<void> toggleVideo() async {
    ...
    _liveVideoSubscription = _videoInput.startStreamingImages().listen((frame) {
      if (_isVoiceMode) {
        _geminiLiveService.sendImage(base64Encode(frame));
      }
    });
    ...
    _appVisionTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
       ...
       try {
         ...
         if (decoded != null) {
            ...
            _geminiLiveService.sendImage(base64Encode(jpegBytes));
         }
       } catch (e) {
  ...
  Widget buildVoiceOverlay(BuildContext context, ThemeData theme) {
    final bottomPad = MediaQuery.of(context).padding.bottom + 48;
    final statusText = _isMicMuted ? 'Mic muted' : 'Listening...';
    ...
    return Container(
      ...
      child: Stack(
        ...
        children: [
            ...
            Opacity(
              ...
              child: SizedBox.expand(
                child: FittedBox(
                  fit: BoxFit.cover,
                  child: SizedBox(
                    width: _videoInput.cameraController!.value.previewSize?.height ?? 1,
                    height: _videoInput.cameraController!.value.previewSize?.width ?? 1,
          ...
          if (_isVideoEnabled || _isAppVisionEnabled)
            Positioned(
          ...
          // ---- Center: loading, error, or status pill ----
            Center(
              child: _liveVoiceErrorMessage != null
                  ...
                  : _isLoading
                      ? Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 20),
                            Text('Connecting...', style: GoogleFonts.plusJakartaSans(color: Colors.white54, fontSize: 18)),
                          ],
                        )
                      : Container(
                          ...
                          child: Text(statusText, style: GoogleFonts.plusJakartaSans(color: Colors.white70, fontSize: 16)),
                        ),
            ),
            ...
            child: Row(
              ...
              children: [
                ...
                _buildLabeledButton(
                  ...
                  color: _isVideoEnabled ? Colors.blueAccent : Colors.white24,
                  label: 'Lab',
                  onPressed: () {
                     if (_isAppVisionEnabled) toggleAppVision(); // disable copilot if running
                     toggleVideo();
                  },
                ),
                _buildLabeledButton(
                  icon: _isAppVisionEnabled ? CupertinoIcons.eyeglasses : CupertinoIcons.eye,
                  color: _isAppVisionEnabled ? Colors.purpleAccent : Colors.white24,
                  label: 'Co-Pilot',
                  onPressed: () {
                     if (_isVideoEnabled) toggleVideo(); // disable lab mode if running
                     toggleAppVision();
                  },
                ),
                if (_isVideoEnabled)
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: const Icon(CupertinoIcons.bolt_slash_fill, color: Colors.redAccent, size: 40),
          ),
          ...
          Text(
            liveVoiceErrorMessage ?? 'An unexpected error occurred.',
            textAlign: TextAlign.center,
            style: GoogleFonts.plusJakartaSans(color: Colors.white60, fontSize: 13, height: 1.5),
          ),
          ...
          Row(
            ...
            children: [
              TextButton(
                onPressed: stopLiveVoiceMode,
                child: Text('Close', style: GoogleFonts.plusJakartaSans(color: Colors.white38)),
              ),
              const SizedBox(width: 16),
              ElevatedButton.icon(
                onPressed: () {
                  // Reconnect: close existing and start fresh
                  stopLiveVoiceMode();
                ...
                label: Text('Try Reconnecting', style: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.bold)),
                style: ElevatedButton.styleFrom(
                  backgroundColor: Colors.blueAccent,
                  foregroundColor: Colors.white,
                  padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
