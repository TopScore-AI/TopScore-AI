...
final GoRouter router = GoRouter(
  ...
  redirect: (context, state) {
    ...
    // 1. If we are still initializing (initial boot), show splash/wait.
    // If we are logged in but just fetching profile, we can still redirect.
    ...
    // 1. If we are still initializing or fetching profile, allow splash/current screen to stay.
    if (authProvider.isLoading) return null;

    // 2. Guest mode — let them through to the app freely.
    if (authProvider.isGuestMode) return null;

    // 3. Not logged in? Send to the welcome/solver screen first to reduce friction.
    if (!isLoggedIn) {
      return isLoggingIn ? null : '/guest-welcome';
    }

    // 3. Logged in, but needs email verification?
    if (authProvider.requiresEmailVerification) {
      return isVerifyingEmail ? null : '/verify-email';
    }

    // 4. Logged in, but missing profile info (and hasn't finished onboarding)?
    if (!authProvider.isProfileComplete) {
      return isOnboarding ? null : '/onboarding';
    }

    // 5. Logged in AND Profile Complete, but trying to access login/onboarding/guest-welcome?
    // Redirect them to the Home.
    if (isLoggingIn || isOnboarding || isVerifyingEmail || isGuestWelcome) {
      return '/home';
    }

    // 6. Otherwise, let them go where they asked
    return null;
  },
  routes: [
    GoRoute(
