...
class FirebaseFile {
  ...
  factory FirebaseFile.fromFirestore(DocumentSnapshot doc) {
    ...
    // Safely parse grade/level which might be String or Int in Firestore
    int? parsedGrade;
    final rawGrade = data['grade'] ?? data['level'];
    if (rawGrade != null) {
      if (rawGrade is int) {
        parsedGrade = rawGrade;
      } else if (rawGrade is String) {
        parsedGrade = int.tryParse(rawGrade.replaceAll(RegExp(r'[^0-9]'), ''));
      }
    }

    // Safely parse size which might be String, Int or Double 
    int? parsedSize;
    final rawSize = data['size'] ?? data['fileSize'];
    if (rawSize != null) {
      if (rawSize is int) {
        parsedSize = rawSize;
