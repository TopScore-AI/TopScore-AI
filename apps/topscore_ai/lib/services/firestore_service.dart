...
class FirestoreService {
  ...
  Future<PaginatedResult<ResourceModel>> getPaginatedResources(
    ...
    try {
      ...
      if (kDebugMode) debugPrint('âŒ getPaginatedResources error: $e');
  ...
  Future<List<ResourceModel>> getRecentDriveResources(int limit,
    ...
    try {
      ...
      if (kDebugMode) debugPrint("Error fetching recent drive resources: $e");
      return [];
    }
  }
  ...
  Future<List<UserModel>> getChildren(List<String> childrenIds) async {
    ...
    try {
      ...
      if (kDebugMode) debugPrint("Error fetching children: $e");
      return [];
    }
  }
  ...
  /// Track an AI Tutor session open.
  Future<void> trackTutorSession({
    required String userId,
    ...
    try {
      ...
      if (kDebugMode) debugPrint("Error tracking tutor session: $e");
    }
  }

  /// Fetch recent file-open activity for a user from Firestore.
  Future<List<Map<String, dynamic>>> getFileHistory(
    String userId, {
    int limit = 20,
  }) async {
    try {
      final snapshot = await _firestore
          ...
          .where('userId', isEqualTo: userId)
          .where('action', isEqualTo: 'open_file')
          .orderBy('timestamp', descending: true)
          .limit(limit)
          .get();
      return snapshot.docs.map((doc) => doc.data()).toList();
    } catch (e) {
      if (kDebugMode) debugPrint("Error fetching file history: $e");
      return [];
    }
  }
