...
class ChatController extends ChangeNotifier {
  ...
  Uint8List? get pendingFileBytes => _pendingAttachments.isNotEmpty ? _pendingAttachments.first.bytes : null;
  String? get pendingPreviewData => _pendingAttachments.isNotEmpty ? _pendingAttachments.first.previewData : null;
  String? get pendingFileType => _pendingAttachments.isNotEmpty ? _pendingAttachments.first.type : null;
  String? get pendingFileName => _pendingAttachments.isNotEmpty ? _pendingAttachments.first.name : null;
  String? get pendingFileId => _pendingAttachments.isNotEmpty ? _pendingAttachments.first.id : null;
  ...
  /// THE LIFELINE: Recovers image if Android killed the app while the camera/picker was open.
  Future<void> recoverLostImageIfKilled() async {
    if (kIsWeb) return;
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      try {
        ...
        if (response.file != null) {
          developer.log('📸 Recovered lost image from OS background kill!');
          _handleInitialImageExternal(response.file!);
        } else if (response.exception != null) {
          developer.log('❌ LostDataResponse error: ${response.exception}');
        }
      } catch (e) {
        developer.log('❌ Error retrieving lost data: $e');
      }
    }
  }

  void handleInitialResources({XFile? image, String? text}) {
