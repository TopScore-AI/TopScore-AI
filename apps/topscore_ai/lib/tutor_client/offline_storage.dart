...
class OfflineStorage {
  ...
  Future<List<CachedMessage>> getCachedMessages(String threadId) async {
    ...
    try {
      ...
      if (kDebugMode) debugPrint('Error loading cached messages: $e');
      return [];
    }
  }
  ...
  Future<void> savePendingMessage(PendingMessage message) async {
    ...
    if (kDebugMode) debugPrint('Saved pending message: ${message.id}');
  }
  ...
  Future<List<PendingMessage>> getPendingMessages() async {
    ...
    try {
      ...
      if (kDebugMode) debugPrint('Error loading pending messages: $e');
      return [];
    }
  }
    ...
    try {
      ...
      for (final message in pending) {
        try {
          ...
          if (kDebugMode) debugPrint('Failed to sync message ${message.id}: $e');
          failed++;
        }
      }
    } finally {
