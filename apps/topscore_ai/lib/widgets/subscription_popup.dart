...
class _SubscriptionPopupState extends State<SubscriptionPopup> {
  ...
  Future<void> _initiatePayment() async {
    ...
    try {
      ...
      if (kIsWeb) {
        success =
            checkoutResult is PaystackVerifyResult && checkoutResult.isSuccess;
      } else {
        success =
            checkoutResult is PaystackCheckoutResult && checkoutResult.success;
      }

      if (success && mounted) {
        final days = _isMonthly ? 30 : 7;
        await authProvider.updateSubscription(days);
                ...
                child: ElevatedButton(
                  ...
                  child: _isLoading
                      ? const SizedBox(
                          ...
                          height: 24,
                          child:
                              AppSpinner(color: Colors.white, strokeWidth: 2))
                      : Text(
                          "Upgrade Now",
                          style: GoogleFonts.plusJakartaSans(
                            fontSize: 16,
